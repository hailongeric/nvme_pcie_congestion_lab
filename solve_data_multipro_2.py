...
def solve_data(data_size,gap_num,per_size,lock):
 ...
 #time_line_400_16_2
    ...
    lock.acquire()
    print("\n")
    print("data_size : {}\ngap_num : {}\nper_size : {}".format(data_size,gap_num,per_size))
    print("high index: {}\nlow index: {}".format(high_index,low_index))
    print("high wave duration mean: {}\nhigh wave duration median: {}".format(numpy.mean(high_duration_arry),med1))
    print("low wave duration mean: {}\nlow wave duration median: {}".format(numpy.mean(low_duration_arry),med2))
    print("bandwith : {}\n".format(1/(med1+med2)))
    #print(get_data)
    ...
    for per_size in per_size_arr:
        for data_size in data_size_arr:
            for gap_num in gap_num_arr:
                if process_num < 8:
                    p = Process(target=solve_data,args=(data_size,gap_num,per_size,mutex))
                    p.start()
